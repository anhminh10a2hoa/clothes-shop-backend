...
const _checkJwt = async (req: Request, res: Response, next: any) => {
	...
	try {
		jwtPayload = await (<any>jwt.verify(jwtCookieToken, process.env.JWT_TOKEN ? process.env.JWT_TOKEN : 'abcdef123'))
	...
	const jwtToken = jwt.sign({ id, username, roll }, process.env.JWT_TOKEN ? process.env.JWT_TOKEN : 'abcdef123', {
	...
	res.cookie('jwt', jwtToken, {
		expires: new Date(Date.now() + (process.env.JWT_TOKEN ? process.env.JWT_TOKEN : 'abcdef123')),
		// secure:true,
		httpOnly: true,
	})
